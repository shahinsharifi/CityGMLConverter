...
public class CityKmlImporter implements EventHandler {
	...
	private CityKmlImportWorkerFactory _CityKmlImportWorker;
	...
	public boolean doProcess(KMLObject _kmlObj) {
		...
		while (shouldRun && fileCounter < importFiles.size()) {
			...
			try {
				...
				_CityKmlImportWorker = new CityKmlImportWorkerFactory(dbPool, 
						jaxbBuilder,
						tmpXlinkPool, 
						lookupServerManager, 
						importFilter,
				...
				featureWorkerPool = new WorkerPool<CityGMLChunk>(
						minThreads,
						maxThreads,
						new CityKmlFeatureReaderWorkerFactory(dbWorkerPool, config, eventDispatcher),
						queueSize,
						false);
