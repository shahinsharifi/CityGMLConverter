 ...
 */
package de.tub.citydb.modules.citykml.gui.preferences;
...
import de.tub.citydb.config.project.CitykmlExporter.AltitudeMode;
import de.tub.citydb.config.project.CitykmlExporter.AltitudeOffsetMode;
...
public class AltitudePanel extends AbstractPreferencesComponent {
	...
	public boolean isModified() {
		if (!modeComboBox.getSelectedItem().equals(config.getProject().getCityKmlExporter().getAltitudeMode()))
			return true;

		double altitudeOffsetValue = 0;
		try {
			altitudeOffsetValue = Double.parseDouble(constantOffsetText.getText().trim());
			if (altitudeOffsetValue != config.getProject().getCityKmlExporter().getAltitudeOffsetValue())
				return true;
		}
		...
		switch (config.getProject().getCityKmlExporter().getAltitudeOffsetMode()) {
			case NO_OFFSET:
		...
		if (callGElevationService.isSelected() != config.getProject().getCityKmlExporter().isCallGElevationService())
			return true;

		if (useOriginalZCoords.isSelected() != config.getProject().getCityKmlExporter().isUseOriginalZCoords())
			return true;
	...
	public void doTranslation() {
		...
		modeComboBox.setSelectedItem(config.getProject().getCityKmlExporter().getAltitudeMode());
	...
	public void loadSettings() {
		modeComboBox.setSelectedItem(config.getProject().getCityKmlExporter().getAltitudeMode());
		constantOffsetText.setText(String.valueOf(config.getProject().getCityKmlExporter().getAltitudeOffsetValue()));
		switch (config.getProject().getCityKmlExporter().getAltitudeOffsetMode()) {
			case NO_OFFSET:
		...
		callGElevationService.setSelected(config.getProject().getCityKmlExporter().isCallGElevationService());
		useOriginalZCoords.setSelected(config.getProject().getCityKmlExporter().isUseOriginalZCoords());
		setEnabledComponents();
	}

	@Override
	public void setSettings() {
		config.getProject().getCityKmlExporter().setAltitudeMode((AltitudeMode)modeComboBox.getSelectedItem());

		double altitudeOffsetValue = 0;
		try {
			altitudeOffsetValue = Double.parseDouble(constantOffsetText.getText().trim());
			config.getProject().getCityKmlExporter().setAltitudeOffsetValue(altitudeOffsetValue);
		}
		catch (NumberFormatException nfe) {
		...
		if (noOffsetRadioButton.isSelected()) {
			config.getProject().getCityKmlExporter().setAltitudeOffsetMode(AltitudeOffsetMode.NO_OFFSET);
		}
		else if (constantOffsetRadioButton.isSelected()) {
			config.getProject().getCityKmlExporter().setAltitudeOffsetMode(AltitudeOffsetMode.CONSTANT);
		}
		else if (genericAttributeRadioButton.isSelected()) {
			config.getProject().getCityKmlExporter().setAltitudeOffsetMode(AltitudeOffsetMode.GENERIC_ATTRIBUTE);
		}
		config.getProject().getCityKmlExporter().setCallGElevationService(callGElevationService.isSelected());
		config.getProject().getCityKmlExporter().setUseOriginalZCoords(useOriginalZCoords.isSelected());
	}
