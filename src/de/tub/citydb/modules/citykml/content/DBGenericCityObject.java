...
public class DBGenericCityObject implements Importer {
	...
	public DBGenericCityObject(Connection batchConn, Config config, ImporterManager dbImporterManager) throws SQLException {
	...
	private void init() throws SQLException {
		...
		surfaceGeometryImporter = (SurfaceGeometry)dbImporterManager.getDBImporter(ImporterEnum.SURFACE_GEOMETRY);
		cityObjectImporter = (CityObject)dbImporterManager.getDBImporter(ImporterEnum.CITYOBJECT);
		implicitGeometryImporter = (DBImplicitGeometry)dbImporterManager.getDBImporter(ImporterEnum.IMPLICIT_GEOMETRY);
		stGeometry = (StGeometry)dbImporterManager.getDBImporter(ImporterEnum.ST_GEOMETRY);
		genericAttributeImporter = (DBCityObjectGenericAttrib)dbImporterManager.getDBImporter(ImporterEnum.CITYOBJECT_GENERICATTRIB);
	}
	...
	private boolean insert(GenericCityObject genericCityObject, long genericCityObjectId) throws SQLException {
		...
		if (++batchCounter == Internal.POSTGRESQL_MAX_BATCH_SIZE)
			dbImporterManager.executeBatch(ImporterEnum.GENERIC_CITYOBJECT);
	...
	public ImporterEnum getDBImporterType() {
		return ImporterEnum.GENERIC_CITYOBJECT;
	}
