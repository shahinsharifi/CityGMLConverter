...
public class DBImplicitGeometry implements Importer {
	...
	public DBImplicitGeometry(Connection batchConn, Config config, ImporterManager dbImporterManager) throws SQLException {
	...
	public long insert(ImplicitGeometry implicitGeometry, long parentId) throws SQLException {
		...
		try {
			...
			if (batchCounter > 0) {
				dbImporterManager.executeBatch(ImporterEnum.IMPLICIT_GEOMETRY);
				batchConn.commit();
			}
	...
	public ImporterEnum getDBImporterType() {
		return ImporterEnum.IMPLICIT_GEOMETRY;
	}
}
